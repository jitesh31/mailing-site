   ...
   str=str+", "+d.getDate()+"/"+(d.getMonth()+1)+"/"+(d.getYear()+1900)+","+d.getHours()+":"+d.getMinutes()+":"+d.getSeconds();	
	...
	try
	{
		...
		<%
           ...
           ps=con.prepareStatement("insert into messagedet(from1,to1,subject,message,date,status)values(?, ?, ?, ?, ?, ?)");
		 ...
		 ps.executeUpdate();
		/*   ps=con.prepareStatement("insert into senddet(from1,to1,subject,message,date,status)values(?, ?, ?, ?, ?, ?)");
	       ps.setString(1,from1);
		   ps.setString(2,to1);
		   ps.setString(3,subject);
		   ps.setString(4,message);
